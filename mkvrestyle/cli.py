...
from mkvrestyle.banner import cli_banner
...
from mkvrestyle.helper import read_file, combine_arguments_by_batch
from mkvrestyle.process import ProcessDisplay
...
def prepare_track_info(file, index, codec, lang):
    return (
        ...
        + "_"
        + lang
        + subtitle_extension_for_mimetype(codec)
    )
...
def extract_subtitles_and_fonts(
    ...
    """
    Extracts subtitles and fonts from an input file.

    Args: input_file (Path): The path to the input file. attachments_folder (Path): The path to the folder where
    attachments will be saved. stream_select (str|int|None): The stream selection option. If "None", the user will be
    prompted to select a subtitle stream. If a numeric value, it will be used as the subtitle index. If a language
    code, the subtitle stream with that language will be selected.

    Returns:
        tuple: A tuple containing two lists. The first list contains the following elements:
            - The path to the extracted subtitle file.
            - The path to the extracted ASS track file.
            - A list of available fonts.
        The second list is empty if there are no attachments. If there are attachments, it contains a list of
        dictionaries with the following keys:
            - file_path (Path): The path to the font file.
            - font_name (str): The name of the font.
            - font_family (str): The family of the font.
            - font_style (str): The style of the font.
            - font_weight (str): The weight of the font.
            - font_stretch (str): The stretch of the font.
            - font_file_size (int): The size of the font file in bytes.
            - font_file_hash (str): The hash of the font file.

    Raises:
        SubtitleNotFoundError: If no subtitle stream is found in the input file.
    """
...
def subtitle_extension_for_mimetype(codec_id):
...
def main(input_path, output_path, preset, stream):
...
def cli():
    """
    A tool for basic restyling of an embedded ASS file with a new user-defined font and styling.

    Documentation: https://github.com/ToshY/mkvrestyle
    """

    cli_banner("mkvrestyle")
